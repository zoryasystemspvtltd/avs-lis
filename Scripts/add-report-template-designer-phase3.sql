-- Phase 3: Report Template Designer schema extensions (idempotent)
-- Does not change production print behavior.
...
IF NOT EXISTS (SELECT 1 FROM dbo.ReportTemplateModeSetting WHERE ReportType = N'Diagnostic')
    ...
    VALUES (N'Diagnostic', N'SystemDefault', N'phase3-seed', GETUTCDATE());
GO
IF NOT EXISTS (SELECT 1 FROM dbo.ReportTemplateModeSetting WHERE ReportType = N'Radiology')
    INSERT INTO dbo.ReportTemplateModeSetting (ReportType, Mode, ModifiedBy, ModifiedOn)
    VALUES (N'Radiology', N'SystemDefault', N'phase3-seed', GETUTCDATE());
GO

UPDATE dbo.ReportTemplate
SET TemplateCategory = N'SystemDefault'
