...
function H([string]$token, [string]$json = $null) {
...
function PostJson($uri, $token, $body) {
  Invoke-RestMethod -Method Post -Uri $uri -Headers (H $token) -Body ($body | ConvertTo-Json -Depth 10 -Compress)
}
...
try {
  ...
  $paramCode = "UAT-P-$tag".Substring(0, [Math]::Min(20, "UAT-P-$tag".Length))
  ...
  $loaded = Invoke-RestMethod -Uri "$baseApi/api/HisParameterMaster/$paramId" -Headers (H $token)
  ...
  $reloaded = Invoke-RestMethod -Uri "$baseApi/api/HisParameterMaster/$paramId" -Headers (H $token)
...
try {
  $nextPid = (Invoke-RestMethod -Uri "$baseApi/api/PatientMaster/NextPatientId" -Headers (H $token)).patientId
  Expect-Error { PostJson "$baseApi/api/PatientMaster" $token @{ Name = "UAT NoPhone $tag"; HisPatientId = $nextPid; Phone = ""; Gender = "M"; Age = 30; DateOfBirth = "1996-01-01"; IsActive = $true } } "Phone"
  ...
  $patLoaded = Invoke-RestMethod -Uri "$baseApi/api/PatientMaster/$patientId" -Headers (H $token)
...
try {
  ...
  $invNo = Invoke-RestMethod -Uri "$baseApi/api/SaleInvoice/NextInvoiceNo" -Headers (H $token)
  $dto = @{
    invoice = @{
  ...
  $draft = Invoke-RestMethod -Uri "$baseApi/api/SaleInvoice/$invoiceId" -Headers (H $token)
  ...
  $edited = Invoke-RestMethod -Uri "$baseApi/api/SaleInvoice/$invoiceId" -Headers (H $token)
  ...
  $paid = Invoke-RestMethod -Uri "$baseApi/api/SaleInvoice/$invoiceId" -Headers (H $token)
...
try {
  ...
  $collQ = Invoke-RestMethod -Uri "$baseApi/api/SampleCollection/PendingQueue" -Headers (H $token $collOpt)
  ...
  $collAfter = Invoke-RestMethod -Uri "$baseApi/api/SampleCollection/PendingQueue" -Headers (H $token $collOpt)
  ...
  $recvQ = Invoke-RestMethod -Uri "$baseApi/api/SampleReceiving/ReceivingQueue" -Headers (H $token $recvOpt)
  ...
  $recvAfter = Invoke-RestMethod -Uri "$baseApi/api/SampleReceiving/ReceivingQueue" -Headers (H $token $recvOpt)
...
try {
  ...
  $recent = Invoke-RestMethod -Uri "$baseApi/api/Patients" -Headers (H $token $recvOpt)
  ...
  if ($unrecvId -gt 0) {
    ...
    $allRecent = Items (Invoke-RestMethod -Uri "$baseApi/api/Patients" -Headers (H $token $badOpt))
...
try {
  ...
  $tech = Invoke-RestMethod -Uri "$baseApi/api/Patients" -Headers (H $token $techOpt)
  ...
  $recentOnly = Invoke-RestMethod -Uri "$baseApi/api/Patients" -Headers (H $token $recvOnlyTech)
  if ($null -eq $recentOnly) { throw "ReceivedOnly queue null" }

  # Diagnostic report — ensure test request with results path reachable
  $hasResults = [int](SqlScalar "SELECT COUNT(*) FROM TestResult WHERE TestRequestId=$reqId")
